...
def  main():
  ...
  installer = "\installer\KSynth Installer.exe"
   
  if demo:
    installer = "\installer\KSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\KSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "KSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "KSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
